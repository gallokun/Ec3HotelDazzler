...
function consultarClientes($conexionDB){
	...
	if($consulta -> num_rows != 0){
		...
		while ($cliente = $consulta -> fetch_assoc()) {
			...
			$salida .= '
			<tr>
				<td class="center">'.$cliente['nombre_cli'].'</td>
...
function buscarClientes($conexionDB,$nombre){
	...
	$consulta = $conexionDB -> query("SELECT nombre_cli,user,email,tel FROM clientes WHERE nombre_cli LIKE '%$nombre%' ");
...
function consultarReservaciones($conexionDB){
	...
	$consulta = $conexionDB -> query("SELECT reservaciones.id_reservacion,reservaciones.checkin,reservaciones.checkout,
		reservaciones.estado,reservaciones.num_adu,reservaciones.num_niñ,reservaciones.id_cliente,reservaciones.habitacion,
		 clientes.nombre_cli as Cliente,habitaciones.nombre as Habitacion, tipohabitacion.nombre as nombret FROM reservaciones,clientes,habitaciones,tipohabitacion
